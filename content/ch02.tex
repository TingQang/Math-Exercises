...
\begin{Exercise}[title={极限与连续}, label={ex:ch02-01}]
    \Question \difficulty{1} 计算 $\limxa \frac{\sin x}{x}$，并证明其结果。
    ...
    \Question \difficulty{4} 计算 $\limn \left(1 + \frac{1}{n}\right)^n$，并说明其几何意义。
...
\begin{Exercise}[title={函数连续性选择题}, label={ex:ch02-17}]
    ...
    \end{enumerate}
    \matchexampoint{函数连续性}
    \exercisetip{利用连续性的定义判断，即 $\limx0 f(x) = f(0)$}
\end{Exercise}
...
\begin{Exercise}[title={极限计算填空题}, label={ex:ch02-20}]
    \Question \difficulty{2} 计算 $\limx0 \frac{\sin 3x}{x} = $ \underline{\hspace{2cm}}. \exercisesource{考研真题}
    \matchexampoint{极限计算}
    \exercisetip{利用等价无穷小替换或重要极限 $\limx0 \frac{\sin x}{x} = 1$}
\end{Exercise}
...
\begin{Answer}[ref={ex:ch02-01}]
    ...
    $\limx0 \frac{\sin x}{x} = 1$（重要极限公式）
    
    证明：利用夹逼准则，当 $0 < x < \frac{\pi}{2}$ 时，$\sin x < x < \tan x$，两边除以 $\sin x$ 得 $1 < \frac{x}{\sin x} < \frac{1}{\cos x}$，取极限得 $\lim_{x \to 0^+} \frac{\sin x}{x} = 1$，同理 $\lim_{x \to 0^-} \frac{\sin x}{x} = 1$，因此 $\limx0 \frac{\sin x}{x} = 1$。
    ...
    可导性：左导数 
    $\longmath{\lim_{x \to 0^-} \frac{f(x) - f(0)}{x - 0} = \lim_{x \to 0^-} \frac{x^2 - 0}{x} = \lim_{x \to 0^-} x = 0}$，
    右导数 
    $\longmath{\lim_{x \to 0^+} \frac{f(x) - f(0)}{x - 0} = \lim_{x \to 0^+} \frac{\sin x - 0}{x} = 1}$，
    左右导数不相等，因此不可导。
    
    \Question 
    \textbf{易错点：} 重要极限公式的应用
    
    $\limn \left(1 + \frac{1}{n}\right)^n = e$（重要极限公式）
