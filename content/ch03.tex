...
\begin{Exercise}[title={极限的四则运算法则}, label={ex:ch03-01}]
    \Question \difficulty{1} 计算 $\limx0 \frac{x^2 - 1}{x - 1}$ \exercisesource{考研数学一·2018}
...
\begin{Exercise}[title={等价无穷小替换}, label={ex:ch03-02}]
    \Question \difficulty{1} 计算 $\limx0 \frac{\sin 2x}{\sin x}$ \exercisesource{考研数学一·2019}
    \matchexampoint{三角函数极限}
    \exercisetip{利用倍角公式化简}
    
    \Question \difficulty{2} 计算 $\limx0 \frac{e^x - 1}{x}$ \exercisesource{考研数学一·2020}
    \matchexampoint{重要极限公式}
    \exercisetip{利用等价无穷小代换}
    
    \Question \difficulty{2} 计算 $\limx0 \frac{\ln(1 + x)}{x}$ \exercisesource{考研数学一·2021}
    \matchexampoint{重要极限公式}
    \exercisetip{利用等价无穷小代换}
\end{Exercise}
...
\begin{Exercise}[title={洛必达法则}, label={ex:ch03-03}]
    ...
    \Question \difficulty{4} 计算 $\limx \frac{x^2}{\ln x}$ \exercisesource{考研数学一·2019}
...
\begin{Exercise}[title={泰勒公式}, label={ex:ch03-04}]
    \Question \difficulty{3} 计算 $\limx0 \frac{x - \sin x}{x^3}$ \exercisesource{考研数学一·2021}
    \matchexampoint{等价无穷小代换}
    \exercisetip{利用泰勒展开或洛必达法则}
    
    \Question \difficulty{4} 计算 $\limx0 \frac{\sin x - x}{x^3}$ \exercisesource{考研数学一·2021}
    \matchexampoint{等价无穷小代换}
    \exercisetip{利用泰勒展开}
    
    \Question \difficulty{5} 计算 $\limx0 \frac{\tan x - \sin x}{x^3}$ \exercisesource{考研数学一·2019}
    \matchexampoint{等价无穷小代换}
    \exercisetip{利用泰勒展开}
\end{Exercise}
...
\begin{Exercise}[title={重要极限公式}, label={ex:ch03-05}]
    \Question \difficulty{1} 计算 $\limx0 \frac{\sin x}{x}$ \exercisesource{考研数学一·2017}
    \matchexampoint{重要极限公式}
    \exercisetip{直接应用公式}
    
    \Question \difficulty{4} 计算 $\limx (1 + \frac{1}{x})^x$ \exercisesource{考研数学一·2022}
    \matchexampoint{数列极限与函数极限的转化}
    \exercisetip{利用重要极限公式}
    
    \Question \difficulty{4} 计算 $\limx (1 + \frac{2}{x})^x$ \exercisesource{考研数学一·2018}
    \matchexampoint{数列极限与函数极限的转化}
    \exercisetip{利用重要极限公式}
\end{Exercise}
...
\begin{Answer}[ref={ex:ch03-01}] % 对应题型1的标签
    $\limx0 \frac{x^2 - 1}{x - 1} = \limx0 (x + 1) = 1$（直接代入）
...
\begin{Answer}[ref={ex:ch03-02}] % 对应题型2的标签
    $\limx0 \frac{\sin 2x}{\sin x} = \limx0 \frac{2\sin x \cos x}{\sin x} = \limx0 2\cos x = 2$（倍角公式化简）
    
    $\limx0 \frac{e^x - 1}{x} = 1$（等价无穷小代换：$e^x - 1 \sim x$）
    
    $\limx0 \frac{\ln(1 + x)}{x} = 1$（等价无穷小代换：$\ln(1 + x) \sim x$）
\end{Answer}
